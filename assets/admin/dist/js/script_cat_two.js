$(document).ready(function() {
    ...
    $("#add_pole_cat_lang").on('click', function () {
        ...
        $('.content').animate({
            scrollTop: $('#add_cat_two').offset().top
        }, 'slow');
    ...
    $(".delete_cat").on('click', function(){
        var form= $("#forma_del")[0];
        var vse_polya= new FormData(form);
        var url = $('.url').val();
        $.ajax({
            method: "POST",
            url: url+"admin/requestCatTwo/delete_cat_two",
            data: vse_polya,
            contentType: false,
            processData: false,
        }).done(function(){
            window.location=url+"/admin/catalogs_two";
        });
    });
