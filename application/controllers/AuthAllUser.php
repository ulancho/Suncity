...
class AuthAllUser extends CI_Controller {
	...
	{
		if(logged_in()) redirect('authalluser/dash');
        ...
        $acn = $this->lang->line('text_noacaunt_sumbit');
		...
		if($this->form_validation->run()){
			if($this->authitalluser->login(set_value('email'), set_value('password'))){
				// Redirect to your logged in landing page here
		...
		$this->load->view('all_user_auth/login', $data);
	}
	...
	public function signup()
	{
        ...
        $pass = $this->lang->line('param_pass');
        $param_fullname = $this->lang->line('param_fullname');
        $param_phnumber = $this->lang->line('param_phnumber');
        $param_passwds = $this->lang->line('param_passwds');
        $text_confpass_sumbit = $this->lang->line('text_confpass_sumbit');
        ...
        $data['error'] = '';
        $data['pass'] = $pass;
        $data['param_fullname'] = $param_fullname;
        $data['param_phnumber'] = $param_phnumber;
        $data['conf_passwds'] = $text_confpass_sumbit;
        $data['sumbit'] = $voiti;
        $data['sign'] = $sign;
        $data['acn'] = $acn;

		$this->form_validation->set_rules('fullname', "$param_fullname", 'required');
		$this->form_validation->set_rules('tel_number', $param_phnumber, 'required');

		$this->form_validation->set_rules('email', 'Email', 'required|valid_email|is_unique['. $this->config->item('authit_all_users_table') .'.email]');
		$this->form_validation->set_rules('password', "$pass", 'required|min_length['. $this->config->item('authit_password_min_length') .']');
		$this->form_validation->set_rules('password_conf', "$param_passwds", 'required|matches[password]');
		
		if($this->form_validation->run()){
			if($this->authitalluser->signup(set_value('fullname'),set_value('email') , set_value('password'),set_value('tel_number'))){
				$this->authitalluser->login(set_value('email'), set_value('password'));
		...
		$this->load->view('all_user_auth/signup', $data);
	}
